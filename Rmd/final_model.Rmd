...
Since after cleaning the dataset, we are remained with 2030 valid entries, we can still assume that we can train a valid model from this dataset.
...
summary(tf_reduced_time_model4)
```
...
ggplot(plot_data_combined, aes(x = "Distribution", y = Value)) +
  ...
  labs(
    ...
    subtitle = "Gold Area = Explained Variance (R²); Blue Area = Unexplained Variance",
...
```{r}
library(zoo) # Required for as.yearm  
